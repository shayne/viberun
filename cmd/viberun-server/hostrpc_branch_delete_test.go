...
func TestHostRPCBranchDeleteAttachFromBranch(t *testing.T) {
	baseDir := t.TempDir()
	...
	attachApp := ""
	attachAction := ""
	attachSrv := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.Method != http.MethodPost || r.URL.Path != "/attach" {
			...
			attachAction = strings.TrimSpace(r.Form.Get("action"))
	...
	req := httptest.NewRequest(http.MethodPost, "http://unix/branch/delete?attach=1", nil)
	...
	if gotBase != "myapp" || gotBranch != "rainbow" {
		t.Fatalf("unexpected delete args: %q %q", gotBase, gotBranch)
	}
	if attachApp != "myapp" {
		t.Fatalf("expected attach app myapp, got %q", attachApp)
	}
	if attachAction != "" {
		t.Fatalf("expected empty attach action, got %q", attachAction)
	}
}
