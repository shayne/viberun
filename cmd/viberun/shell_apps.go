...
func ensureForwardForSummary(state *shellState, summary appSummary) appSummary {
	if state == nil {
		return summary
	}
	...
	if forward, ok := state.appForwards[summary.Name]; ok {
		if forward.port == summary.Port && forward.err == nil {
	...
	if state.gateway == nil {
		...
		state.appForwards[summary.Name] = appForward{port: summary.Port, err: errors.New("gateway not connected")}
		return summary
	}
	if err := ensureLocalPortAvailable(summary.Port); err != nil {
		summary.ForwardErr = forwardErrorMessage(err)
		state.appForwards[summary.Name] = appForward{port: summary.Port, err: err}
		return summary
	}
	...
	summary.Forwarded = true
	summary.ForwardErr = ""
	state.appForwards[summary.Name] = appForward{port: summary.Port, close: closeFn}
	return summary
}
...
func openAppURL(state *shellState, app string) (string, error) {
	...
	summary = ensureForwardForSummary(state, summary)
	if ok {
