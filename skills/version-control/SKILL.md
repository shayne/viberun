...
3) If SSH:
   - From laptop: start the shell with `VIBERUN_FORWARD_AGENT=1` (for example, `VIBERUN_FORWARD_AGENT=1 viberun`).
   - For existing apps, run `viberun`, then `app <app>` and `update` to recreate the container with the agent socket mounted.
...
## Notes
- If the git identity is missing, update it locally and restart the container (or run `viberun`, then `app <app>` and `update`) so the startup config applies.
