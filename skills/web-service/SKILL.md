...
9) After the service is confirmed healthy:
   ...
   - If no URL is set, ask if the user wants to allow public access:
     - If yes, tell them to run `app <app>` then `url public` from their laptop (inside the viberun shell).
     - If no, say: “You can do this later from your laptop with `app <app>` then `url public` in the viberun shell.”
...
- Before risky or destructive changes (removing services, deleting data, large refactors), ask the user if they want a snapshot.
  ...
  - If host snapshots are unavailable, ask the user to run `app <app>` then `snapshot` from their machine (inside the viberun shell).
...
- If the user asks to roll back, run `vrctl host snapshots` to list available tags, then `vrctl host restore <ref>`.
  ...
  - Restore will disconnect the session; tell the user to re-run `viberun`, then `vibe <app>`, and then re-check service status / bring it back up.
  - If restore is unavailable, ask the user to run `app <app>` then `restore <ref>` from their machine.
...
- If the URL is set, say it requires login by default and can be made public with `app <app>` then `url public`.
...
- After verifying the service is responding, if no URL is available, run `xdg-open "http://localhost:${port}"` once.
